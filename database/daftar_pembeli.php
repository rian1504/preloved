<?php
if (!$_POST) {
    header("location: ../pembeli/daftar.php");
} else {
    include "koneksi.php";
    ...
    // ambil data username
    $query_username = "SELECT username FROM pengguna WHERE peran='pembeli' AND username='$username'";
    ...
    if ($username == $data_username["username"]) {
        header("location: ../pembeli/daftar.php?pesan=gagal_username");
    } else {

        // Validasi email
        $is_email = filter_var($email, FILTER_VALIDATE_EMAIL);
        if (!$is_email) {
            header("location: ../pembeli/daftar.php?pesan=gagal_email");
            exit;
        }

        // Validasi password
        if ($password != $konfirmasi_password) {
            header("location: ../pembeli/daftar.php?pesan=gagal_konfirmasi");
            exit;
        }
        ...
        // SQL Prepare
        $query_tambah_pengguna = "INSERT INTO `pengguna`(nama, email, alamat, no_hp, tanggal_lahir, username, password, peran) VALUES(?, ?, ?, ?, ?, ?, ?, 'pembeli')";
        $stmt = mysqli_prepare($koneksi, $query_tambah_pengguna);

        if ($stmt) {
            mysqli_stmt_bind_param($stmt, "sssssss", $nama, $email, $alamat, $no_hp, $tanggal_lahir, $username, $password);
            $result = mysqli_stmt_execute($stmt);

            if ($result) {
                header("location: ../tamu/login.php?pesan=berhasil_daftar");
            } else {
                header("location: ../pembeli/daftar.php?pesan=gagal_daftar");
            }
